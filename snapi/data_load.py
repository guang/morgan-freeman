...
class Net2DataFlow(DataFlow):
    def get_data(self):
        ...
        snapi_logger.warn("whooaaahh it actually gets used!")
        while True:
            wav_file = random.choice(self.wav_files)
            # yield get_mfccs_and_spectrogram(wav_file)
            yield get_mfccs_and_spectrogram(wav_file, trim=False, random_crop=False)
...
def get_mfccs_and_spectrogram(wav_file, trim=True, random_crop=False):
    ...
    # Padding or crop
    actual_duration = librosa.core.get_duration(y=wav, sr=hp.default.sr)
    length = hp.default.sr * actual_duration
